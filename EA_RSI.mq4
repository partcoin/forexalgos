//+------------------------------------------------------------------+
//|                                                       EA_RSI.mq4 |
//|                                      Copyright © 2013, Forex Ltd.|
//|                                                                  |
//+------------------------------------------------------------------+
#property copyright "Copyright © 2013, Forex Ltd."
#property link      "http://www.forex.com"
#property strict
...
double         pt,blots,slots,mblots, mslots,sl,tp;
double         rsi3, nextbuylevel, nextselllevel;
int            mt,expire=0,dg,totalbuy,totalsell,total,totalbuystop,totalsellstop,totalpending,ticket,i,cnt;
...
int OnInit()
{
   EventSetTimer(Delay);                      // create timer events / set frequency of timer events in seconds
...
void OnTimer()
 {
   if ( IsExpertEnabled() && AlwaysOn )
    {
      int mainReturn = main();  // run the expert's code
    }
 }
...
//Orders manager
   ...
   if(total<1&&totalpending>0){newdelete();}
   
//Breakeven      
   if(BreakevenGain>0)
      { bool breakEvenMoved = movebreakeven(BreakevenGain,Breakeven); }

//Spread      
   returnValue++;
   if(MaxSpread!=0)if((Ask-Bid)>MaxSpread*pt)
      return(returnValue);
...
//Maximum open orders
   returnValue++;
   if(MaxOpenPositions<100)if((total)>=MaxOpenPositions)
     return(returnValue);
...
bool checktime()
{
   string svrdate=IntegerToString(Year())+"."+IntegerToString(Month())+"."+IntegerToString(Day());
      ...
      if( (( (nstarthour<=nendhour) && (TimeCurrent()<tstart) ) || ( (TimeCurrent()>tend) || (nstarthour>nendhour) )) && ((TimeCurrent()<tstart) && (TimeCurrent()>tend)) )   // THIS LOOOKS SUSPICIOUS
         return(true);
   //
   return(false);
}
...
int count(int type)
{
   ...
   if(OrdersTotal()>0)
   {
      ...
      {
         bool selectSucceeded = OrderSelect(i,SELECT_BY_POS,MODE_TRADES);
...
void hideclose(int type)
{
   ...
   if(OrdersTotal()>0)
   {
      ...
      {
         bool slecteSucceeded = OrderSelect(i,SELECT_BY_POS,MODE_TRADES);
         if(type==OP_BUY && OrderType()==OP_BUY)
         {
            if(  ( (OrderSymbol()==Symbol()) && (OrderMagicNumber()==MagicNumber) && HideSL && (StopLoss>0)
                   && (OrderProfit() <= (((-1)*StopLoss*OrderLots()*10-MarketInfo(Symbol(),MODE_SPREAD)*OrderLots()*10)/mt))
                 )
              ...
              )
            {
               ...
               result = OrderClose(OrderTicket(),OrderLots(),NormalizeDouble(Bid,Digits),Slippage*mt, clrNONE);
...
void close(int type)
{
   ...
   if(OrdersTotal()>0)
   {
      for(i=OrdersTotal()-1;i>=0;i--)
      {
         bool succeeded = OrderSelect(i,SELECT_BY_POS,MODE_TRADES);
         if(type==OP_BUY && OrderType()==OP_BUY)
         {
...
void newdelete()
{
   if(OrdersTotal()>0)
   {
      for(i=OrdersTotal();i>=0;i--)
      {
         ...
         {
            succeeded = OrderDelete(OrderTicket());
         }
      }
   }
}
...
//Breakeven
bool movebreakeven(double breakevengain,double breakeven)
{
   RefreshRates();
   if(OrdersTotal()>0)
   {
         ...
         if( (OrderType()<=OP_SELL) && (OrderSymbol()==Symbol()) && (OrderMagicNumber()==MagicNumber) )
         {
            if(OrderType()==OP_BUY)
            {
               if(NormalizeDouble((Bid-OrderOpenPrice()),Digits)>=NormalizeDouble(breakevengain*pt,Digits))
               {
                  ...
                  {
                     succeeded = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(OrderOpenPrice()+breakeven*pt,Digits),OrderTakeProfit(),0,Blue);
                     return(true);
                  }
               }
            }
            else
            {
               if(NormalizeDouble((OrderOpenPrice()-Ask),Digits)>=NormalizeDouble(breakevengain*pt,Digits))
               {
                  ...
                  {
                     succeeded = OrderModify(OrderTicket(),OrderOpenPrice(),NormalizeDouble(OrderOpenPrice()-breakeven*pt,Digits),OrderTakeProfit(),0,Red);
                     return(true);
                  }
               }
            }
...
//Open Orders
int open(int type,double lots,double price,double stoploss,double takeprofit,int localEexpire,color clr)
{
   int localTicket=0;
   ...
   if(type==OP_SELL || type==OP_SELLSTOP || type==OP_SELLLIMIT)
   {
      ...
      localTicket=OrderSend(Symbol(),type,NormalizeDouble(lots,LotDigits),NormalizeDouble(price,Digits),Slippage*mt,sl,tp,EAName+" - "+DoubleToStr(MagicNumber,0),MagicNumber,localEexpire,clr);
